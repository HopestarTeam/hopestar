...
public class MaterialDisplay : MonoBehaviour
{
    ...
    private void GetResourceValues(){   //modify this function to get the right resource type   !!!!!!
        resourceProduction = Mathf.Min(GameManager.gm.variables.variables[GlobalVariableEnum.Material].production, 30);
        resourceSpent = GameManager.gm.variables.variables[GlobalVariableEnum.Material].spent;
        resourceInUpkeep = GameManager.gm.variables.variables[GlobalVariableEnum.Material].upkeep;
    }

    private void UpdateDisplay(){    
        //we check if the values have changed, if they have not, we don't need to refresh the display
        if (resourceProduction !=  GameManager.gm.variables.variables[GlobalVariableEnum.Material].production
         || resourceSpent != GameManager.gm.variables.variables[GlobalVariableEnum.Material].spent
         || resourceInUpkeep != GameManager.gm.variables.variables[GlobalVariableEnum.Material].upkeep
        ){
            GetResourceValues();
    ...
    private void DisplayIcons(){
        ...
        for (int i = 0; i < resourceProduction; i++){
            ...
            newIcon.GetComponent<ResourceIcon>().SetIcons(
                ...
                Resources.Load<Sprite>("Icons/material_locked")
            );
