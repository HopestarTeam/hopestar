...
public class ScienceDisplay : MonoBehaviour
{
    ...
    private void GetResourceValues(){   //modify this function to get the right resource type   !!!!!!
        resourceProduction = Mathf.Min(GameManager.gm.variables.variables[GlobalVariableEnum.Science].production, 30);
        resourceSpent = GameManager.gm.variables.variables[GlobalVariableEnum.Science].spent;
        resourceInUpkeep = GameManager.gm.variables.variables[GlobalVariableEnum.Science].upkeep;
    }

    private void UpdateDisplay(){    
        //we check if the values have changed, if they have not, we don't need to refresh the display
        if (resourceProduction !=  GameManager.gm.variables.variables[GlobalVariableEnum.Science].production
         || resourceSpent != GameManager.gm.variables.variables[GlobalVariableEnum.Science].spent
         || resourceInUpkeep != GameManager.gm.variables.variables[GlobalVariableEnum.Science].upkeep
        ){
            GetResourceValues();
    ...
    private void DisplayIcons(){
        ...
        for (int i = 0; i < resourceProduction; i++){
            ...
            newIcon.GetComponent<ResourceIcon>().SetIcons(
                ...
                Resources.Load<Sprite>("Icons/science_locked")
            );
